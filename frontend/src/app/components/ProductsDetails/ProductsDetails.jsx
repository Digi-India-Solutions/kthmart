...
//   const getPricePerPiece = (qty) => {
//     if (!book?.package?.length) return Number(book?.finalPrice) || 0;
  
//     const sorted = [...book.package].sort((a, b) => a.stock - b.stock);
  
//     let price = Number(book.finalPrice);
...
//      {book?.package?.map((pkg, index) => (
//        <div key={index} className="flex justify-between items-center pb-2 border-b last:border-none ">
...
//          <button
//          onClick={() => {
//           setSelectedPackage(pkg.stock);   // highlight selected tier
//           updateQuantity(pkg.stock);       // update qty & cart
//         }}
        
   
//            className={`font-medium px-3 py-1 rounded flex items-center gap-1 cursor-pointer
//             ${selectedPackage === pkg.stock
//               ? "text-green-600" : "text-red-500"}`}
//          >
//            {selectedPackage === pkg.stock
// ? "✔ Selected" : `Add ${pkg.stock}`}
//          </button>
//        </div>
//      ))}
//    </div>
...
//                     ₹{ (quantity * getPricePerPiece(quantity))?.toFixed(0) }
...
//   ₹{getPricePerPiece(quantity)} per piece
// </div>
...
export default function ProductDetails() {
  ...
  // FIX: Selected package highlight
  const [selectedPackage, setSelectedPackage] = useState(null);

  // FIX: Package quantity state
  const [quantity, setQuantity] = useState(0);
  ...
  // FIX: book must be defined BEFORE price logic
  const [book, setBook] = useState(null);
  ...
  if (!book) return <div className="text-center">Product not found</div>;

  // PRICE PER PIECE LOGIC
  const getPricePerPiece = (qty) => {
    try {
      const base = Number(book.finalPrice) || 0;
      const tiers = book.package || [];
      ...
      let price = base;
      sorted.forEach((tier) => {
        if (qty >= Number(tier.stock)) price = Number(tier.price);
      });

      return price;
    } catch {
      return Number(book.finalPrice) || 0;
    }
  };

  // ACTIVE PACKAGE (shows checkmark)
  const getActivePackage = (qty) => {
    const tiers = book?.package || [];
  ...
  // HIDE LOWER TIERS
  const shouldHidePackage = (qty, index) => {
    const activeIndex = getActivePackage(qty);
    return activeIndex !== null && index < activeIndex;
  };

  // QUANTITY UPDATE + AUTO UPDATE CART
  const updateQuantity = (newQty) => {
    const qty = Math.max(0, newQty);
    setQuantity(qty);
  ...
  const handleAddToCart = (product) => {
    ...
    const cartItem = {
      ...
      name: product.title,
      image: product.images[0],
      price: getPricePerPiece(useQty),
      finalPrice: getPricePerPiece(useQty),
      quantity: useQty,
    };
  ...
  // WISHLIST LOGIC
  const handleAddToWishlist = (id, title, img, fp, price) => {
    const isLogged = Boolean(user?.email);

    const already = isLogged
      ? wishlistItems.some((i) => i._id === id)
    ...
    if (already) {
      ...
      if (isLogged) {
        dispatch(addToWishlistState({ id }));
        dispatch(addToWishlistApi({ productId: id }));
      } else {
        dispatch(addToWishlist({ id, name: title, image: img, price: fp, oldPrice: price }));
      }
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between">
            ...
            <div className="flex gap-2">
              <button
                className="p-2 border border-green-600 rounded-md hover:bg-green-100"
                onClick={() =>
                  handleAddToWishlist(
                    book._id,
                    book.title,
              ...
              >
                {wishlistItems.some((item) =>
                  user?.email ? item?._id === book._id : item?.id === book._id
                )
                  ? "❤️"
                  : <Heart size={20} />}
              </button>

              <button className="p-2 border border-green-600 rounded-md hover:bg-green-100">
                <Share2 />
              </button>
            </div>
          </div>
  ...
  book?.package.length > 0 && (
    <div className="bg-blue-50 p-3 rounded-xl space-y-2">
      {book?.package
        ...
        ?.map((pkg, index) => {
          ...
          {book?.stock > 0 && (
            <button
              className={`${
                cartItems.some((item) => item.id === book._id)
                  ? "w-full bg-black text-white font-medium py-3 rounded-md"
                  : "w-full bg-green-600 hover:bg-green-700 text-white font-medium py-3 rounded-md"
              }`}
              onClick={() => handleAddToCart(book)}
            >
              {cartItems.some((item) => item.id === book._id)
        ...
        {/* RIGHT COLUMN — SAME UI */}
        <div className="space-y-6">
          <div className="flex border-b border-green-700">
            <button className="px-4 py-2 font-medium text-sm">Details</button>
          </div>
      ...
      <div className="mt-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="flex gap-4">
            <Image src={fastdelivery} alt="" className="w-12 h-12"/>
            <p>Superfast Delivery</p>
          </div>

          <div className="flex gap-4">
            <Image src={bestprice} alt="" className="w-12 h-12"/>
            <p>Best Prices & Offers</p>
          </div>

          <div className="flex gap-4">
            <Image src={wide} alt="" className="w-12 h-12"/>
            <p>Wide Assortment</p>
          </div>
        </div>
      </div>
